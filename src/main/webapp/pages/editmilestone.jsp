...
<form id="add" action="<%=request.getContextPath() %>/updatemilestone" method="post">
    ...
    <input type="text" id="taskText" name="taskText" minlength="3" value="${milestone.taskText}"
           pattern="^[a-zA-Z0-9]+$" title="Please enter only letters and numbers.">
    <br>
    <label for="taskDate">Task Date:</label>
    <input type="date" id="taskDate" name="taskDate" value="${milestone.taskDate}" min="<%=java.time.LocalDate.now()%>">
    <br>
    <label for="taskTime">Task Time:</label>
    <input type="time" id="taskTime" name="taskTime" value="${milestone.taskTime}">
    <br>
    <label for="isReminder">Is Reminder:</label>
    <input type="checkbox" id="isReminder" name="isReminder" ${milestone.isRemainder ? 'checked' : ''}>
    <br>
